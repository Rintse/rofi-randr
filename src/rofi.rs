...
impl ListItem {
    pub fn rofi_print(&self) {
        ...
        let cmt = if self.comments.is_empty() { String::new() }
        else {
...
impl From<&OutputEntry> for ListItem {
    fn from(output: &OutputEntry) -> Self {
        let (icon, comments) = match (output.connected, output.enabled) {
            (false,_) => (Icon::Disconnected, vec!["disconnected".to_string()]),
...
impl From<&ResolutionEntry> for ListItem {
    fn from(res_entry : &ResolutionEntry) -> Self {
        let comments = 
            if res_entry.current { vec![ "Current".to_string() ] } 
            else { Vec::new() };
...
impl From<&RateEntry> for ListItem {
    fn from(rate_entry : &RateEntry) -> Self {
        let comments = 
            if rate_entry.current { vec![ "Current".to_string() ] } 
            else { Vec::new() };
...
impl ParseResult<Action> {
    ...
    pub fn output_list(backend: &mut Box<dyn DisplayBackend>) 
    ...
    pub fn relation_list(backend: &mut Box<dyn DisplayBackend>) -> Self {
        let list = backend.supported_relations().into_iter()
            .map(ListItem::from).collect();
    ...
    pub fn rate_list(backend: &mut Box<dyn DisplayBackend>, output : &str) 
    -> Result<Self, AppError> {
    ...
    pub fn resolution_list(backend: &mut Box<dyn DisplayBackend>, output: &str)
    -> Result<Self, AppError> 
    {
        let mut resolutions = backend.get_resolutions(output)?;
        
        // Sort (reversed) by total pixel count
        let res_cmp = |m1: &Resolution, m2: &Resolution| { u64::cmp(
    ...
    ) -> Result<Self, AppError> 
    {
        ...
        let mut others: Vec<&OutputEntry> = outputs.iter()
    ...
    -> Self {
        ...
        let op_list = supported_ops.into_iter()
            .map(ListItem::from)
            .collect();
